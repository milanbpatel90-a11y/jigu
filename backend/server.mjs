...
const s3Endpoint =
  ...
  process.env.WASABI_ENDPOINT_URI ||
...
console.log(
  `S3 Config: endpoint=${s3Endpoint}, region=${s3Region}, bucket=${BUCKET}`,
);
console.log(
  `AWS Credentials: ${hasValidCredentials ? "VALID" : "LOCAL MODE ENABLED"}`,
);
...
const optionalAuth = REQUIRE_AUTH
  ? authMiddleware("read")
  : (req, res, next) => next();
const writeAuth = REQUIRE_AUTH
...
app.post(
  "/upload-model",
  writeAuth,
  upload.fields([{ name: "file" }, { name: "thumb" }]),
  async (req, res) => {
    try {
...
app.post("/rebuild-embeddings", writeAuth, async (req, res) => {
  try {
    ...
    const py = spawn(pythonCmd, ["match.py", "--build"], {
      cwd: process.cwd(),
    });
    let out = "",
      errOut = "";
    ...
    py.on("close", (code) => {
      if (code !== 0)
        return res
          .status(500)
          .json({ error: "Rebuild failed", details: errOut || out });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log("Running vision_matcher.py with files:", filePaths);

      // Create models cache for Python script
      try {
        const data = await s3
          .listObjectsV2({ Bucket: BUCKET, MaxKeys: 500 })
          .promise();
        const glbModels = (data.Contents || [])
          .filter((f) => f.Key.toLowerCase().endsWith(".glb"))
          .filter(
            (f) =>
              !f.Key.toLowerCase().includes("man") &&
              !f.Key.toLowerCase().includes("woman"),
          )
          .map((f) => f.Key);
      ...
      const pythonCmd = process.platform === "win32" ? "python" : "python3";
      // Use vision_matcher.py for AI-based visual similarity matching
      const py = spawn(pythonCmd, ["vision_matcher.py", ...filePaths], {
        cwd: process.cwd(),
      });
      let out = "",
        errOut = "";
      ...
      py.stderr.on("data", (d) => {
        errOut += d.toString();
        console.log("🔍 AI Matcher:", d.toString().trim());
      });
      py.on("close", async (code) => {
        ...
        if (code !== 0)
          return res
            .status(500)
            .json({ error: "AI matching failed", details: errOut || out });
        try {
          ...
          if (jsonOut.error) {
            ...
            const statusCode =
              jsonOut.method === "validation_rejected" ? 400 : 500;
          ...
          if (jsonOut.best_model) {
            if (hasValidCredentials) {
              try {
                ...
                if (modelKey === "default.glb") {
                  try {
                    const data = await s3
                      ...
                      .promise();
                    const glbFiles = (data.Contents || []).filter((f) =>
                      f.Key.toLowerCase().endsWith(".glb"),
                    );
                    if (glbFiles.length > 0) {
                      modelKey = glbFiles[0].Key;
                      jsonOut.best_model = modelKey;
                      console.log(
                        "Using first available model from S3:",
                        modelKey,
                      );
                    }
                  } catch (listErr) {
                ...
                jsonOut.model_url = `${baseUrl}/proxy-model/${encodeURIComponent(modelKey)}`;
                console.log("Generated proxy model URL for:", modelKey);
              } catch (s3Error) {
                console.log("S3 URL generation failed, using local fallback");
                jsonOut.model_url =
                  "http://localhost:5000/local-models/glasses2.glb";
              }
            } else {
...
app.post("/saved-models", optionalAuth, async (req, res) => {
  try {
    ...
    if (!name || (!glbUrl && !url)) {
      return res
        .status(400)
        .json({ error: "name and glbUrl/url are required" });
    }
...
async function downloadReferenceImages() {
  ...
  try {
    const data = await s3
      .listObjectsV2({ Bucket: BUCKET, Prefix: "reference_images/" })
      .promise();
    const images = (data.Contents || []).filter((f) => !f.Key.endsWith("/"));

    if (images.length === 0) {
      console.log(
        "No reference images found in S3 (prefix: reference_images/)",
      );
      return;
    }
    ...
    console.log(
      `Found ${images.length} reference images in S3, ${existingFiles.length} locally`,
    );
